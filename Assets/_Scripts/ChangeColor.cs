...
[ExecuteInEditMode]
[AddComponentMenu ("Custom/ColorRGB")]
[System.Serializable]
public class ChangeColor : MonoBehaviour
{
	[HideInInspector] public Material ForceMaterial;
	[HideInInspector] public bool ActiveChange=true;
	private string shader = "Custom/ColorRGB";
	[HideInInspector] [Range(0, 1)] public float _Alpha = 1f;

	[HideInInspector] public Color _TintColor = new Color (1,1,1,1);
	...
	void Update()
	{
		...
		if (ActiveChange)
		{
			...
			{				
				...
				this.GetComponent<Renderer>().sharedMaterial.SetColor("_Color", _TintColor);
			...
			else if(this.gameObject.GetComponent<Image>() != null)
			{
				...
				CanvasImage.material.SetColor("_Color", _TintColor);
...
public class ChangeColorEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		ChangeColor _changeColorScript = (ChangeColor)target;
		...
		if (_changeColorScript.ForceMaterial == null)
		{
			_changeColorScript.ActiveChange = true;
		}
		else
		{
			if(GUILayout.Button("Remove Shared Material"))
			{
				_changeColorScript.ForceMaterial= null;
				_changeColorScript.ShaderChange = 1;
				_changeColorScript.ActiveChange = true;
				_changeColorScript.CallUpdate();
			}

			EditorGUILayout.PropertyField (m_object.FindProperty ("ActiveChange"), new GUIContent ("Change Material Property", "Change The Material Property"));
		}

		if (_changeColorScript.ActiveChange)
		{

			EditorGUILayout.BeginVertical("Box");

			//EditorGUILayout.PropertyField (m_object.FindProperty(""));

			Texture2D icone = Resources.Load ("2dxfx-icon-red") as Texture2D;
			EditorGUILayout.PropertyField(m_object.FindProperty("_TintColor"), new GUIContent("Tint Color Property", icone, "Change the color value"));

			icone = Resources.Load ("2dxfx-icon-red") as Texture2D;
